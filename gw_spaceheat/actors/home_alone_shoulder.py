...
class HomeAlone(ScadaActor):
    ...
    transitions = (
        [
        # Initializing
        {"trigger": "BufferNeedsCharge", "source": "Initializing", "dest": "HpOnStoreOff"},
        ...
        while not self._stop_requested:
            ...
            if not self.top_state == HomeAloneTopState.Monitor:
                ...
                if self.top_state == HomeAloneTopState.Normal:
                    if self.house_is_cold_onpeak() and self.is_buffer_empty(really_empty=True):
    ...
    def engage_brain(self, waking_up: bool = False) -> None:
        ...
        if not (self.forecasts and self.temperatures_available):
            ...
            if self.state==HomeAloneState.Initializing:
                ...
                if self.is_onpeak():
                    self.trigger_normal_event(HomeAloneEvent.OnPeakStart)
                elif self.is_buffer_full() or self.is_buffer_ready():
                    self.trigger_normal_event(HomeAloneEvent.BufferFull)
    ...
    def get_latest_temperatures(self):
        ...
        required_buffer_energy = self.data.latest_channel_values[H0N.required_energy]
        if total_usable_kwh is None or required_buffer_energy is None:
            self.temperatures_available = False
