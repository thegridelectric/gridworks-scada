...
it to five-v-boss by the last segment of ToHandle, five-v-boss forwards it
to the cycler as its boss, and the cycler opens vdc-relay under the
command's own TriggerId. The loop's second
...
def gwadmin_reboot_dispatch(app: ScadaApp, timeout_seconds: int) -> AdminDispatch:
    """The five-v-boss row's reboot action: the client learns the row from
    the scada's capabilities and sends RebootPicos in the row's own vocabulary."""
