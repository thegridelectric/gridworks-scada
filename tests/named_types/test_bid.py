import json

from gwsproto.enums import MarketPriceUnit, MarketQuantityUnit
from gwsproto.named_types import Bid


def test_bid() -> None:
    d = {
        "BidderAlias": "d1.isone.ver.keene.holly",
        "MarketSlotName": "e.rt60gate5.d1.isone.ver.keene.1667880000",
        "PqPairs": [
            {
                "PriceX1000": 40000,
                "QuantityX1000": 10000,
                "TypeName": "price.quantity.unitless",
                "Version": "001",
            }
        ],
        "InjectionIsPositive": False,
        "PriceUnit": "USDPerMWh",
        "QuantityUnit": "AvgkW",
        "SignedMarketFeeTxn": "",
        "TypeName": "bid",
        "Version": "000",
    }

    t = Bid.model_validate(d).model_dump_json(exclude_none=True)
    d2 = json.loads(t)
    assert d2 == d

    ######################################
    # Enum related
    ######################################

    assert type(d2["PriceUnit"]) is str

    d2 = dict(d, PriceUnit="unknown_enum_thing")
    assert Bid(**d2).PriceUnit == MarketPriceUnit.default()

    assert type(d2["QuantityUnit"]) is str

    d2 = dict(d, QuantityUnit="unknown_enum_thing")
    assert Bid(**d2).QuantityUnit == MarketQuantityUnit.default()
